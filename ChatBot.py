...
class TransformerModel(nn.Module):
    ...
    def __init__(self, v_size, d_model, n_heads, num_layers, max_seq_length):
        ...
        self.transformer = nn.Transformer(d_model, n_heads, num_layers, num_layers)
    ...
    def forward(self, source, target):
        ...
        src = self.embedding(source) + self.positional_encoding[:, source.size(1)]
        tgt = self.embedding(target) + self.positional_encoding[:, target.size(1)]
...
train_dl = DataLoader(dataset=train_ds, batch_size=4, shuffle=True, drop_last=True)

# Initialize the model
model = TransformerModel(v_size=tokenizer.vocab_size, d_model=512, n_heads=8, num_layers=6, max_seq_length=128)
