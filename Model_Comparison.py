...
def Compute_Coherence_BERTopic():
    ...
    texts = [doc.split() for doc in contexts]  # Split your documents into tokens
    ...
    topics = [[word for word, _ in topic_words[topic]] for topic in topic_words]
...
def Compute_Coherence_LDA(from_n_topics, to_n_topics):
    ...
    for context in contexts:
        ...
        base_words = [lemmatizer.lemmatize(token) for token in tokens if token.isalpha()]
        filtered_base_words = [word for word in base_words if word not in custom_stop_words]
    ...
    corpus_bow = [vocabulary.doc2bow(question) for question in processed_questions]
    ...
    for n_topics in range(from_n_topics, to_n_topics):
        ...
        coherence_model_lda = CoherenceModel(model= lda_model, texts=processed_questions, dictionary=vocabulary, coherence='c_v')
        ...
        # Visualize the topics
        vis = pyLDAvis.gensim_models.prepare(lda_model, corpus_tfidf, vocabulary)
